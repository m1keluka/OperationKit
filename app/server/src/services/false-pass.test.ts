...
describe('recordReopenFalsePass', () => {
  it('inserts a gate_false_pass row linking the reopen to the prior passing review', () => {
    const objId = makeObjective('operator')
    const reviewId = addReview(objId, 1, 'pass')

    const rowId = recordReopenFalsePass({ id: objId, workspace: 'operator', agent_context: 'cto' })
    ...
    expect(row.workspace).toBe('operator')
  ...
  it('does NOT fire when the most-recent verdict was fail', () => {
    ...
    expect(recordReopenFalsePass({ id: objId, workspace: 'operator' })).toBeNull()
  ...
  it('is idempotent: a second reopen of the same passing review does not double-insert', () => {
    ...
    expect(recordReopenFalsePass({ id: objId, workspace: 'operator' })).not.toBeNull()
    expect(recordReopenFalsePass({ id: objId, workspace: 'operator' })).toBeNull() // guarded
...
describe('getFalsePassRate', () => {
  it('computes false_passes / pass_gated_reviews per workspace', () => {
    // operator: 2 pass-gated reviews, 1 reopened (false pass) → 0.5
    const a = makeObjective('operator')
    addReview(a, 1, 'pass')
    recordReopenFalsePass({ id: a, workspace: 'operator' })
    const b = makeObjective('operator')
    ...
    const luka = rates.find(r => r.workspace === 'operator')!
  ...
  it('excludes reviews outside the rolling window', () => {
    ...
    const rates = getFalsePassRate(30)
    expect(rates.find(r => r.workspace === 'operator')).toBeUndefined()
  })
})
