// Spawns and supervises the Telegram Contactbook sibling process (Phase 5 Personal CRM).
//
...
//
// Skipped silently if TELEGRAM_BOT_TOKEN or TELEGRAM_CONTACTBOOK_OWNER_ID is unset.
...
// `/home/operator/projects/operationkit/app/telegram-contactbook/index.ts`, which
...
const SIBLING_ENTRY =
  process.env.CONTACTBOOK_SIBLING_ENTRY || `${CC_REPO_DIR}/app/telegram-contactbook/index.ts`
...
export function startContactbookSibling(): void {
  if (!process.env.TELEGRAM_BOT_TOKEN || !process.env.TELEGRAM_CONTACTBOOK_OWNER_ID) {
    console.log('[contactbook-supervisor] disabled — TELEGRAM_BOT_TOKEN or TELEGRAM_CONTACTBOOK_OWNER_ID not set')
    return
  }
  if (!process.env.ANTHROPIC_API_KEY) {
    console.warn('[contactbook-supervisor] ANTHROPIC_API_KEY missing — sibling will exit immediately')
  }
  if (!fs.existsSync(SIBLING_ENTRY)) {
    console.warn(`[contactbook-supervisor] entry not found: ${SIBLING_ENTRY} (skipping)`)
    return
  }
  spawnOnce()
}
...
function spawnOnce(): void {
  ...
  console.log(`[contactbook-supervisor] spawning sibling: tsx ${SIBLING_ENTRY}`)
  ...
  child.on('exit', (code, signal) => {
    ...
    console.log(`[contactbook-supervisor] sibling exited code=${code} signal=${signal} after ${ranFor}ms`)
    if (stopping) return
  ...
  child.on('error', err => {
    console.error('[contactbook-supervisor] spawn error:', err)
  })
}
...
export function stopContactbookSibling(): void {
  stopping = true
  if (child && !child.killed) child.kill('SIGTERM')
}

process.on('SIGTERM', stopContactbookSibling)
process.on('SIGINT', stopContactbookSibling)
