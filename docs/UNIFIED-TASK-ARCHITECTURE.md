...
Single-source-of-truth map of the **capture → dispatch → execute → archive** loop that keeps work flowing through the OperationKit board without manual task management.

This document was produced as part of OperationKit objective #141 ("Build unified task management architecture", 2026-06-04). It catalogs every component, identifies which wires are connected vs missing, and records what was fixed in that session vs what remains as follow-up.
...
```
 ...
 │                              EXECUTE                                     │
 │                                                                          │
 │  OperationKit (this app, port 3002)                                      │
 ...
 │      live OperationKit objective state + recent git activity             │
 │                                                                          │
 │  Per-day (23:00 ET cron):                                                │
 │    • generate-daily-digest.sh synthesises workspaces/<ws>/daily/         │
 │      YYYY-MM-DD.md across OperationKit activity + git + audits + costs   │
 │                                                                          │
...
|---|---|---|---|---|
| Hermes Telegram gateway | `/home/operator/.hermes/`, systemd `hermes-gateway` | Telegram bot DM | `POST /api/internal/objectives` (OperationKit), `/api/internal/vault/*` (rolodex) | **Planned** — venv setup script in `scripts/setup-hermes.sh`; not yet deployed (no `/home/operator/.hermes` on disk, no systemd unit) |
| Telegram Rolodex | `app/telegram-rolodex/index.ts` | Operator DMs | OperationKit vault tools (search/append/update on `~/second-brain/`) | **Planned** — same systemd unit as Hermes |
...
| Hermes seed memory | `scripts/hermes-seeds/MEMORY.md` (OperationKit API contract + decomposition heuristics) + `USER.md` (the operator's profile) | **Authored** and ready to load into Hermes' system prompt the moment the service comes up |
...
| `update-active-state.sh` | Hourly | Rewrites `workspaces/{example,example-project}/active.md` + `personal/active.md` from OperationKit API + git | **Working** (cron + cron.env on host, last run validated 2026-06-04) |
| `generate-daily-digest.sh` | 23:00 ET | Synthesises `workspaces/<ws>/daily/YYYY-MM-DD.md` from OperationKit + git + audits + transcripts | **Working** when container is up |
...
3. OperationKit returns three `id`s; Hermes replies *"Created 3 objectives: #N1 (cto/example), #N2 (general/example), #N3 (cto/example)"*.
...
**Status today:** steps 2–6 work; step 1 is the missing link (Hermes systemd unit not deployed). Until Hermes is running, the operator must curl the endpoint directly or use the OperationKit web UI to create objectives.
...
3. OperationKit frontend polls `GET /api/meeting-queue/count` and shows a badge.
...
## 4. Gap Closures in This Session (OperationKit #141)
...
| Daily/internal briefing queried invalid statuses (`needs_review`, `blocked`) and always returned empty `needsReview` array | Rewrote queries to use `('working','review')`; derived `blocked` from `has_blockers` column | `app/server/src/routes/jarvis.ts:28-54`, `app/server/src/routes/internal.ts:391-401` |
...
These were intentionally left for separate OperationKit objectives — each is meaningful enough to plan and own independently rather than smuggle into this session.
...
2. **Install missing host cron entries** — granola ingest (every 15 min) and gmail-triage (every 30 min). Both wrappers exist; only the crontab lines are missing. Format:
   ...
   */30 * * * * . /home/operator/.config/command-center/cron.env && bash ${CC_REPO_DIR:-/home/operator/projects/operationkit}/scripts/run-gmail-triage.sh >> /home/operator/transcripts/gmail-triage.cron.log 2>&1 # command-center: gmail-triage
   ```
3. **Live-email → objective bridge** — when Gmail triage classifies an envelope as `Live` and the sender is unknown, create a `general`-agent OperationKit objective so it can't fall through the cracks. Probably belongs in `gmail-triage.ts:runGmailTriage` after the label apply.
...
6. **Himalaya → OperationKit integration** — IMAP envelope classification stops at Gmail labels. Either route ACTION envelopes to the same Live-email bridge from #3, or roll Himalaya into Gmail triage.
...
- **Every brain dump becomes a board card** — the OperationKit `objectives` table is the only place running work lives. If you find yourself tracking work in a notepad, Slack, or a stray markdown file, you're outside the loop.
...
- Source objective: OperationKit #141
- Sibling objective (running in parallel): OperationKit #142 ("Close and archive stale second brain tasks", `coo`)
