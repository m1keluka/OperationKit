...
- **Evaluator model** — defaults to **Opus** (`claude-opus-4-8`), consistent with the
  Mike all-Opus rule. Future A/B: the Kitchen Loop uses Haiku as the "weakest-evaluator /
...
VALUES ('uat_gate_config:command-center-infra', '{"enabled":true}')
...
DELETE FROM settings WHERE key = 'uat_gate_config:command-center-infra';
```
