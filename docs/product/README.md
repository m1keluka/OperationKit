...
**In a paragraph:** You open a card, pick a model, and an agent session starts on the VPS. The card is the source of truth: queue → working → review → done. Jarvis is how you talk to the system. Jobs are cards that fire on a schedule. Content, Docs, and Dashboard are the other daily tools. GitHub and Google connect per person, so Ava’s mail and PRs are Ava’s. Subscription seats (Claude / SuperGrok / ChatGPT) are shared operator accounts, not user logins.
